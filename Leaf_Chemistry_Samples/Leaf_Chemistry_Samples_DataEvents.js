...
/*****************************************************************
            ...
            FUNCTIONS DATA EVENTS LEAF CHEMISTRY SAMPLES
...
/*
 DRAFT test
Record link:    site box_id sample
Date:           
Text:           
multi choice:   
repeatable:     
...
function callback(event) {
  ...
  ******************************/
  if (event.name === 'load-record') {
    ...
    changeValues();
    if (!ISBLANK($sample_id)) {
      SETVALUE('bottle_id', $sample_id);
      SETVALUE('bag_id', CONCATENATE($sample_id, '-C'));
    }
  }
  ...
  if (event.name === 'change'){
    ...
    if (event.field === 'sample') {
      ...
      if (!ISBLANK($sample)) {
        ...
        SETVALUE('bottle_id', null);
        SETVALUE('bag_id', null);
      }
    }
    if (event.field === 'sample_id') {
      if (!ISBLANK($sample_id)) {
        SETVALUE('bottle_id', $sample_id);
        SETVALUE('bag_id', CONCATENATE($sample_id, '-C'));
      }
...
ON('change','sample_id', callback);
ON('change','sample_status', callback);
